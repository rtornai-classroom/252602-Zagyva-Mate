...
// Kamera
float cameraR = 9.0f; // Sugar (8 es 10 közt)
...
void genSphere(float r, int slices, int stacks)
{
    for (int i = 0; i <= stacks; ++i)
    {
        ...
        for (int j = 0; j <= slices; ++j)
        {
            ...
            spherePos.push_back(glm::vec3(r * sin(phi) * cos(theta),
...
{
    ShaderInfo shaders[] = { {GL_VERTEX_SHADER, "vertexShader.glsl", 0},
    ...
    glBufferData(GL_ARRAY_BUFFER, spherePos.size() * sizeof(glm::vec3),
        spherePos.data(), GL_STATIC_DRAW);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
    glEnableVertexAttribArray(0);
    glBindBuffer(GL_ARRAY_BUFFER, BO[2]);
    glBufferData(GL_ARRAY_BUFFER, sphereUV.size() * sizeof(glm::vec2),
        sphereUV.data(), GL_STATIC_DRAW);
    glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, 0, 0);
    glEnableVertexAttribArray(2);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, BO[3]);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sphereIdx.size() * sizeof(unsigned int),
        sphereIdx.data(), GL_STATIC_DRAW);

    texture[0] = SOIL_load_OGL_texture("sun.jpg", SOIL_LOAD_AUTO,
    ...
    glClearColor(0.3f, 0.3f, 0.4f, 1.0f); // Hatter
}
...
void display(GLFWwindow* window, double currentTime)
{
    ...
    glm::vec3 camPos = glm::vec3(cameraR * cos(cameraAngle),
        cameraR * sin(cameraAngle), cameraZ);
    matView = glm::lookAt(camPos, glm::vec3(0, 0, 0),
        glm::vec3(0, 0, 1)); // Kamera origoba nez

    glUniformMatrix4fv(glGetUniformLocation(program[0], "matView"), 1, GL_FALSE,
        glm::value_ptr(matView));
    glUniformMatrix4fv(glGetUniformLocation(program[0], "matProjection"), 1,
        GL_FALSE, glm::value_ptr(matProjection));
    glUniform3fv(glGetUniformLocation(program[0], "cameraPosition"), 1,
        glm::value_ptr(camPos));
    glUniform1i(glGetUniformLocation(program[0], "useLighting"), lightOn);

    float lightR = 2.0f * cameraR;
    glm::vec3 lightPos =
        glm::vec3(lightR * cos(currentTime), lightR * sin(currentTime), 0.0f);
    glUniform3fv(glGetUniformLocation(program[0], "lightPosition"), 1,
        glm::value_ptr(lightPos));

    glUniform1i(glGetUniformLocation(program[0], "useTexture"), 0);
    glUniform3f(glGetUniformLocation(program[0], "objectColor"), 1.0f, 1.0f,
        1.0f);
    glBindVertexArray(VAO[0]);
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        glUniformMatrix4fv(glGetUniformLocation(program[0], "matModel"), 1,
            GL_FALSE, glm::value_ptr(matModel));
        glm::mat3 invTrans = glm::mat3(glm::inverseTranspose(matModel));
        glUniformMatrix3fv(
            glGetUniformLocation(program[0], "inverseTransposeMatrix"), 1, GL_FALSE,
            glm::value_ptr(invTrans));
    ...
    glUniformMatrix4fv(glGetUniformLocation(program[0], "matModel"), 1, GL_FALSE,
...
void framebufferSizeCallback(GLFWwindow* window, int width, int height)
{
    ...
    matProjection = glm::perspective(glm::radians(55.0f), (float)width / height,
        0.1f, 100.0f); // 55 fok
}
...
int main()
{
    ...
    std::cout << "Fel - Le      :  Kamera mozgatasa a Z-tengely menten"
        << std::endl;
